...
- **NEXT = Phương manual QA on v3 preview:** set Cloudflare Pages → Preview env `PUBLIC_R2L_W1=1`, then on `v3.felixbuilderhub.pages.dev` with a real test code: hub hook (<2s, 3G throttle) · create flow end-to-end (12 topic cards → wait scene → Đọc ngay; reload mid-gen no double-burn) · lesson gate + karaoke + anti-guess + recap · `/phu-huynh` in Zalo webview · flag off = legacy identical. Cursor agents share ONE working tree (D:\felixbuilderhub) — future agents should use git worktrees.

## W1 PROMOTED TO MAIN (2026-06-11)
- **`main` = `7102a60`, LIVE + verified** (`/phu-huynh` 200 · hoc-sinh legacy · lesson 200). W1 **DARK in prod** (`PUBLIC_R2L_W1` unset). Chain: `ab6cfc5` promote merge (single `--no-ff`, 204/204) → `ae07407` mic-capture engine (pushed directly to main by another agent mid-promote) → `7102a60` build hotfix.
- **Build incident (resolved):** W1 rank plumbing declared `const upstreamBody` twice in `functions/api/generate-read2lead-pack.js` → esbuild fail; Production builds ab6cfc5 + ae07407 FAILED on Cloudflare while prod kept serving the last good deploy (zero student impact). Local `astro build` does NOT compile `functions/` — **verification gap now closed: always run `npx wrangler pages functions build` before promoting.**
- **ROLLBACK W1 if needed:** `git revert -m 1 ab6cfc5 && git push` (single merge commit) — but note `ae07407` (mic engine) sits on top; reverting W1 alone keeps it. Instant alternative: Cloudflare dashboard → Deployments → previous good deploy → Rollback.
- Mic-capture `ae07407` bypassed rails (direct main push, +410 lines into lesson.astro over W1) — builds/tests green after hotfix (220/220), but its merge with W1 lesson code is UNREVIEWED → QA the speaking/Nói lại flow extra carefully on prod.
