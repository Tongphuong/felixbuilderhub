/**
 * Parent view renderer for the unified profile page.
 * Extracted and adapted from phu-huynh.ts.
 */
import type { ProgressPayload } from './ho-so';
import { escapeHtml } from './ho-so';
...
type PortfolioEntry = {
  ...
  reactions?: { heart?: number; clap?: number; strong?: number };
  video_url?: string;
};
...
// ── Cache for supplementary data ──

let cachedLesson: LessonPayload['lesson'] | null = null;
let cachedPortfolioItems: PortfolioEntry[] = [];
  ...
  const bars = recent.length
    ? recent
        .map((entry) => {
          ...
          return `<div class="parent-quality-bar"><span class="parent-quality-fill ${entry.suspect ? 'parent-quality-fill--warn' : ''}" style="height: ${value}%"></span></div>`;
        })
        .join('')
    : '<p class="parent-muted text-sm">Chưa đủ dữ liệu nói trong 7 ngày.</p>';
  const alert =
    attention > 0 && attention < 50
  ...
  return `
    <section class="parent-section" data-learning-quality>
      ...
      <div class="parent-stat-grid">
        <div class="parent-stat"><p class="parent-stat__value">${firstTryPercent}%</p><p class="parent-stat__label">Qua lần đầu</p></div>
        <div class="parent-stat"><p class="parent-stat__value">${retry}</p><p class="parent-stat__label">Lần thử lại</p></div>
        <div class="parent-stat"><p class="parent-stat__value">${attention}</p><p class="parent-stat__label">Tập trung</p></div>
      </div>
      ...
      .map((item) => {
        ...
        return `
          <article id="video-${id}" class="parent-video-card" data-portfolio-card="${id}">
            ...
            <video class="parent-video mt-3" controls playsinline preload="metadata" src="${escapeHtml(item.video_url || '')}" data-portfolio-id="${id}"></video>
            ${item.note_vi ? `<p class="mt-3 text-sm leading-relaxed text-[#1e2a4a]">${escapeHtml(item.note_vi)}</p>` : ''}
            <div class="parent-reactions mt-3" aria-label="Gửi cảm xúc">
              <button type="button" class="parent-reaction" data-reaction-kind="heart" data-portfolio-id="${id}" aria-label="Thả tim"><span aria-hidden="true">❤️</span><span data-reaction-count="heart">${reactionCount(item, 'heart')}</span></button>
              <button type="button" class="parent-reaction" data-reaction-kind="clap" data-portfolio-id="${id}" aria-label="Vỗ tay"><span aria-hidden="true">👏</span><span data-reaction-count="clap">${reactionCount(item, 'clap')}</span></button>
              <button type="button" class="parent-reaction" data-reaction-kind="strong" data-portfolio-id="${id}" aria-label="Cố lên"><span aria-hidden="true">💪</span><span data-reaction-count="strong">${reactionCount(item, 'strong')}</span></button>
            </div>
          </article>`;
      })
      .join('');
  ...
  root.querySelectorAll<HTMLButtonElement>('button[data-reaction-kind]').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        const payload = (await response.json()) as { ok?: boolean; reactions?: Record<string, number> };
...
function renderFullDashboard(
  ...
  container.innerHTML = `
    <div class="space-y-4">
      <section class="parent-section">
        ...
        <div class="parent-stat-grid">
          <div class="parent-stat"><p class="parent-stat__value" data-clarity-mask="true">${escapeHtml(formatLevel(read2LeadState.current_level || progress.current_level))}</p><p class="parent-stat__label">Cấp đọc</p></div>
          <div class="parent-stat"><p class="parent-stat__value">${Number(read2LeadState.streak_days || 0)}</p><p class="parent-stat__label">Ngày liên tiếp</p></div>
          <div class="parent-stat"><p class="parent-stat__value">${Number(storyProgress.total_completed ?? progress.completed_packs ?? 0)}</p><p class="parent-stat__label">Truyện xong</p></div>
        </div>
  ...
  attachPortfolioInteractions(container, code);
}
...
export async function renderParentView(
  ...
  // Show loading while fetching supplementary data
  container.innerHTML = `
    <div class="text-center py-8">
      <div class="ho-so-spinner"></div>
      <p class="mt-3 text-sm" style="color: #5a6a8c;">Đang tải dữ liệu phụ huynh...</p>
    </div>`;

  // Use cache if available for same code
  if (lastFetchCode === code && (cachedLesson !== null || cachedPortfolioItems.length > 0)) {
    renderFullDashboard(container, data, cachedLesson, code, cachedPortfolioItems, cachedPortfolioFailed);
    return;
  }

  // Fetch lesson + portfolio in parallel
  const [lessonResult, portfolioResult] = await Promise.all([
    (async () => {
      const packId = pack?.pack_id;
      if (!packId || pack.status === 'generation_in_progress') return null;
      try {
        ...
        return res.ok && data.ok ? data.lesson || null : null;
      } catch { return null; }
    })(),
    (async () => {
      try {
        const res = await fetch(`/api/parent/portfolio?code=${encodeURIComponent(code)}`);
        const payload = (await res.json()) as PortfolioPayload;
        if (!res.ok || !payload.ok) return { items: [] as PortfolioEntry[], failed: true };
        return { items: Array.isArray(payload.items) ? payload.items : [], failed: false };
      } catch { return { items: [] as PortfolioEntry[], failed: true }; }
    })(),
  ]);

  // Cache results
  lastFetchCode = code;
  cachedLesson = lessonResult;
  cachedPortfolioItems = portfolioResult.items;
  cachedPortfolioFailed = portfolioResult.failed;
