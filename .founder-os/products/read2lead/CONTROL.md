...
- Design self-verification: PASS — mock at 390/1280 approved by Phương 2026-07-13 (_ops/r2l-home-mock-390.png, -1280.png); deployed-preview render MATCHES the mock at both widths (_ops/r2l-preview-ready-390.png, r2l-home-real-1280.png). Fonts corrected during the mock step: the plan text claimed "Baloo 2", the design system actually uses Manrope/Inter — mock and build use the real tokens.
- Live verification on the DEPLOYED preview (claude-r2l-student-home.felixbuilderhub.pages.dev, real student magic link, student "Ong" / R2L-ONG-U5M6 / L2): ready phase renders 5 buttons with the real access code wired in (speak `/speak-up?code=R2L-ONG-U5M6`, ho-so `/ho-so?code=…`, shop `/read2lead/shop?code=…&v3=1`), token NOT leaked into any href, code stashed in sessionStorage. ONE TAP on Tạo bài đọc → real pack created end-to-end (book "My Family", pack_id f9451f53…, "Mở bài học →" link valid) with NO topic picker. Error phase verified live too (code_exhausted → clean "Chưa tạo được bài học" card + retry + Zalo hint).
- Bug caught by live verification, fixed in 3d1e88a: book packs set `topic` to the book's OWN title, so the result card printed "Chủ đề: My Family" directly above "Tên truyện: My Family". Buffet's static trace had concluded book packs carry no topic and the line would stay hidden — the live run disproved it. Topic line is now suppressed when it merely repeats the story title; re-verified on the redeployed preview (served bundle hash C2goCimC == local build; minified `toLowerCase()===o.toLowerCase()` present; render shows the title exactly once).
- Founder handoff: pending Phương's merge GO. Preview: https://claude-r2l-student-home.felixbuilderhub.pages.dev/r2l/start?t=<student token>. NOTE FOR FOUNDER: the live create-flow test consumed student Ong's LAST lesson credit (her code now reports "Mã đã hết lượt"); she has a real "My Family" lesson waiting but needs a top-up to create another. Post-merge: regenerate student links at 365d/5000.
