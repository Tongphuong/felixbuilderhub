...
test('buildReward common returns coins in 10..20', () => {
  ...
  assert.ok(reward.coins >= 10 && reward.coins <= 20);
  assert.equal(reward.part_id, null);
});

test('buildReward rare returns coins in 25..40', () => {
  const reward = buildReward('rare', () => 0.25);
  assert.ok(reward.coins >= 25 && reward.coins <= 40);
});
...
test('buildReward unknown rarity returns zero reward', () => {
  assert.deepEqual(buildReward('mythic', () => 0.5), { coins: 0, part_id: null });
});

test('rollChest combines roll + build', () => {
  const chest = rollChest(() => 0.01);
  assert.equal(chest.rarity, 'epic');
  assert.equal(typeof chest.reward.coins, 'number');
...
test('autoConvertDuplicate replaces dup part with bonus coins', () => {
  const chest = {
    rarity: 'rare',
    reward: { coins: 30, part_id: 'png-default-eyes-a' },
  };
  const converted = autoConvertDuplicate(chest, ['png-default-eyes-a']);
  assert.equal(converted.duplicate, true);
  assert.equal(converted.reward.coins, 30 + DUPLICATE_CONVERSION.rare);
...
test('autoConvertDuplicate keeps part if not in owned set', () => {
  const chest = {
    rarity: 'epic',
    reward: { coins: 50, part_id: 'png-default-body-bluea' },
  };
...
test('autoConvertDuplicate accepts Set or Array for ownedPartIds', () => {
  const chest = {
    rarity: 'common',
    reward: { coins: 15, part_id: 'dup-part' },
  };
  ...
  assert.equal(fromArray.reward.coins, 15 + DUPLICATE_CONVERSION.common);
});

test('chestPreviewText transparent — exposes ranges', () => {
  assert.match(chestPreviewText('common'), /10–20 xu/);
  assert.match(chestPreviewText('rare'), /25–40 xu/);
  assert.match(chestPreviewText('rare'), /phần thưởng/);
  assert.match(chestPreviewText('epic'), /50 xu/);
  assert.equal(chestPreviewText('unknown'), '');
});

test('CHEST_REWARDS and DUPLICATE_CONVERSION match spec constants', () => {
  assert.equal(CHEST_REWARDS.common.part_pool, null);
  assert.equal(CHEST_REWARDS.epic.coins_min, CHEST_REWARDS.epic.coins_max);
  assert.equal(DUPLICATE_CONVERSION.epic, 60);
});
